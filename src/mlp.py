...
if __name__ == '__main__':
    ...
    # x2 = data['rate']
    # ms = np.hstack((pred_np_x, x2))
    # msave('nx', ms)
    # msave('ny', train_y)
    # pred_y = pred_np.round()
    ...
    # pred_y = pred_np.round()
    # cutoff = get_optimal_cutoff(test_y, pred_np)
    ...
    # n_y = predicted.reshape(-1).detach().numpy()
    # ui = np.array(np.where(n_y >= 0.7))
    # li = np.array(np.where(n_y <= 0.3))
    # i = np.append(ui,li)
